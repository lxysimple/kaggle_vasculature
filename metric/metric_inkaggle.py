...
!python -m pip install --no-index --find-links=/kaggle/input/pip-download-for-segmentation-models-pytorch segmentation-models-pytorch  # 安装分割模型PyTorch库
...
#                 print("img.shape ", img.shape)
                ...
                # [1, 5, 1041, 1511] -> [1, 1041, 1511]，这里1是指batchsize=1
...
#                     mask_count[..., x1:x2, y1:y2] += 1
                ...
                # mask_pred = (mask_pred * 255 * 1.0).to(tc.uint8).cpu()
...
#                 plt.show()
            ...
            if axis == 1:
                mask_list = mask_list.permute(2, 0, 1)
            if axis == 2:
                mask_list = mask_list.permute(1, 2, 0)
...
# # 计算阈值
# # 我感觉是找到一张图中正样本占总样本的比例，然后TH就是这个比例
# # 但这里貌似正样本实在是太少了，导致这个算法算出的TH就是255，不精准
